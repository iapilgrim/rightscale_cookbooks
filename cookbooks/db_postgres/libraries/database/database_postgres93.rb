...
module RightScale
  module Tools
    class DatabasePostgres < RightScale::Tools::MyDatabase
      ...
      def write_backup_info_file(master_uuid, master_ip, is_master)
        ...
        masterstatus['Master_IP'] = node[:db_postgres][:current_master_ip]
        masterstatus['Master_instance_uuid'] = node[:db_postgres][:current_master_uuid]
        if node[:db_postgres][:this_is_master]
        ...
        ::File.open(::File.join(node[:db][:data_dir], RightScale::PostgreSQL::Helper::SNAPSHOT_POSITION_FILENAME), ::File::CREAT|::File::TRUNC|::File::RDWR) do |out|
